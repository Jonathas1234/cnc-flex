  ...
  return (
    <section className="py-28 bg-[#f7f8fa]" id="videos">
      <div className="max-w-7xl mx-auto px-5 sm:px-8">
        ...
        <motion.div className="mb-4 overflow-hidden bg-white border border-[#0f1419]/[0.04] hover:shadow-xl rounded-sm" initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}}>
          <div className="relative" style={{aspectRatio:"16/9"}}>
            ...
            : <><img src={vids[0].thumb} alt={vids[0].title} className="w-full h-full object-cover"/>
              <div className="absolute inset-0 flex items-center justify-center cursor-pointer group" onClick={()=>setPlaying(vids[0].ytId)}>
                <div className="w-20 h-20 flex items-center justify-center rounded-full bg-[#F38104] shadow-2xl group-hover:scale-110 transition-transform"><Play size={28} className="text-white ml-1"/></div>
              </div></>}
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {vids.slice(1).map((v,i)=>(<motion.div key={v.ytId} initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:i*0.08}}
            className="overflow-hidden bg-white border border-[#0f1419]/[0.04] hover:shadow-xl rounded-sm">
            <div className="relative" style={{aspectRatio:"16/9"}}>
              ...
              : <><img src={v.thumb} alt={v.title} className="w-full h-full object-cover"/>
                <div className="absolute inset-0 flex items-center justify-center cursor-pointer group" onClick={()=>setPlaying(v.ytId)}>
                  <div className="w-16 h-16 flex items-center justify-center rounded-full bg-[#0A3C6E] shadow-xl group-hover:scale-110 transition-transform"><Play size={22} className="text-white ml-0.5"/></div>
                </div></>}
            </div>
