...
export function Equipamentos() {
  ...
  const produtos = [
    ...
    {name:"5D Pro Profissional",description:"Fresadora 6 Eixos para Joalheria e Bijuterias",image:"/5dpro.jpg",badge:"6 EIXOS",badgeColor:"#F38104",category:"Joalheria",
          ...
          {list.map((p,i)=>(
            <motion.div key={p.name+filter} initial={{opacity:0,y:30}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.5,delay:i*0.06}}
              ...
              <div className="relative aspect-[4/3] overflow-hidden bg-white p-4">
                <img src={p.image} alt={p.name} className="w-full h-full object-contain transition-transform duration-700 group-hover:scale-105" style={{mixBlendMode:"multiply", filter:"brightness(1.15) contrast(1.05)"}} loading="lazy"/>
