        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {list.map((p,i)=>(
            <motion.div key={p.name+filter} initial={{opacity:0,y:30}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.5,delay:i*0.06}}
              className="group bg-white/60 backdrop-blur-lg border border-white/70 active:bg-white/90 hover:bg-white/80 hover:shadow-[0_12px_40px_rgba(10,60,110,0.1)] overflow-hidden cursor-pointer transition-all duration-700 rounded-xl"
              ...
              <div className="p-6">
                ...
                <div className="space-y-2 mb-6">
                  {p.specs.map((s,j)=>(<div key={j} className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{background:j%2===0?"#0A3C6E":"#F38104"}}/><span className="text-[13px] text-[#0f1419]/60">{s}</span></div>))}
                </div>
