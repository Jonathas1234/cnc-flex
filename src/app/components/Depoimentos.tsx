      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8">
        <div className="text-center mb-16">
          <motion.div className="flex items-center justify-center gap-3 mb-4" initial={{opacity:0}} whileInView={{opacity:1}} viewport={{once:true}}>
            <div className="w-8 h-[1px] bg-[#0A3C6E]"/><span className="text-[11px] font-semibold tracking-[0.2em] uppercase text-[#0A3C6E]">Depoimentos</span><div className="w-8 h-[1px] bg-[#0A3C6E]"/>
          </motion.div>
          <motion.h2 className="text-3xl sm:text-4xl font-bold text-[#0A3C6E]" style={{letterSpacing:"-0.04em"}} initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}}>
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {deps.map((d,i)=>(<motion.div key={i} initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:i*0.1}}
            ...
            <div className="flex items-center gap-3 mt-auto">
              <img src={d.avatar} alt={d.author} className="w-10 h-10 rounded-full flex-shrink-0 ring-2 ring-white/50" loading="lazy"/>
