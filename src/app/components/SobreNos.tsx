    ...
    <section id="sobre" className="py-28 relative overflow-hidden" style={{background:"linear-gradient(135deg, #F38104 0%, #e06f00 40%, #c85f00 100%)"}}>
      ...
      <div className="max-w-4xl mx-auto px-5 sm:px-8 text-center relative z-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {stats.map((s,i)=>(<motion.div key={i} className="p-6 sm:p-8 bg-white/[0.15] backdrop-blur-xl border border-white/[0.25] rounded-2xl hover:bg-white/[0.25] hover:border-white/[0.35] transition-all duration-500 shadow-[0_8px_32px_rgba(0,0,0,0.08)]"
            initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:i*0.08}}>
            <div className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-2" style={{letterSpacing:"-0.03em"}}>{s.n}</div>
            <div className="text-[10px] sm:text-[12px] font-semibold tracking-[0.1em] uppercase text-white/70">{s.l}</div>
          </motion.div>))}
        </div>
        <motion.div className="mt-12" initial={{opacity:0,y:15}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:0.4}}>
          <a href="#contato" onClick={(e)=>{e.preventDefault();document.querySelector("#contato")?.scrollIntoView({behavior:"smooth"})}}
            className="inline-flex px-10 py-4 text-[12px] font-bold tracking-[0.12em] uppercase bg-[#0A3C6E] text-white active:brightness-90 hover:brightness-125 rounded-sm transition-all duration-300 shadow-[0_4px_20px_rgba(10,60,110,0.4)]">
            Fale com a gente
          </a>
        </motion.div>
      </div>
