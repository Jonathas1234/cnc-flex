...
export function ProductModal({isOpen,onClose,product}:ProductModalProps) {
  ...
  return (<AnimatePresence>{isOpen && (<>
    ...
    <motion.div initial={{opacity:0,scale:0.96,y:20}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.96,y:20}} transition={{duration:0.3}}
      ...
      <div className="h-full overflow-y-auto overscroll-contain"><div className="grid lg:grid-cols-2 min-h-full">
        <div className="relative bg-[#f7f8fa]/60 backdrop-blur-sm p-6 sm:p-10 flex items-center justify-center"><img src={product.image} alt={product.name} className="w-full h-auto max-h-[40vh] sm:max-h-[70vh] object-contain" loading="lazy"/></div>
