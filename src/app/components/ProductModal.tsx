...
export function ProductModal({isOpen,onClose,product}:ProductModalProps) {
  ...
  return (<AnimatePresence>{isOpen && (<>
    ...
    <motion.div initial={{opacity:0,scale:0.96,y:20}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.96,y:20}}
      ...
      <button onClick={onClose} className="absolute top-4 right-4 w-9 h-9 flex items-center justify-center border border-[#0f1419]/[0.06] hover:border-[#F38104]/30 text-[#0f1419]/30 hover:text-[#F38104] rounded-sm transition-all z-10"><X size={16}/></button>
      <div className="h-full overflow-y-auto"><div className="grid lg:grid-cols-2 min-h-full">
        <div className="relative bg-[#f7f8fa] p-6 sm:p-10 flex items-center justify-center"><img src={product.image} alt={product.name} className="w-full h-auto max-h-[70vh] object-contain"/></div>
        <div className="p-6 sm:p-10 flex flex-col">
          <div className="inline-block px-3 py-1 text-[9px] font-bold tracking-[0.15em] uppercase mb-5 border border-[#0A3C6E]/20 text-[#0A3C6E] rounded-sm self-start">Profissional</div>
          ...
          <div className="space-y-2 mb-8">
            {product.technicalSpecs.map((s,i)=>(<div key={i} className="flex items-start gap-2.5 p-2.5 bg-[#f7f8fa] rounded-sm">
          ...
          <div className="mt-auto flex flex-col sm:flex-row gap-2.5">
            <a href={`https://wa.me/5511938023558?text=${encodeURIComponent(`Olá! Info sobre ${product.name}`)}`} target="_blank" rel="noopener noreferrer"
              className="flex-1 py-3.5 text-center text-[11px] font-bold tracking-[0.12em] uppercase bg-[#F38104] text-white hover:brightness-110 rounded-sm transition-all">Solicitar Orçamento</a>
            <button onClick={onClose} className="px-6 py-3.5 text-[11px] font-bold tracking-[0.12em] uppercase border border-[#0f1419]/[0.06] text-[#0f1419]/40 hover:text-[#0f1419]/60 rounded-sm transition-all">Fechar</button>
          </div>
        </div>
      </div></div>
