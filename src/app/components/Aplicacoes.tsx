        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {items.map((it,i)=>{const Icon=it.icon;return(
            <motion.div key={i} initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:i*0.04}}
              ...
              <p className="text-[14px] text-[#0f1419]/55 group-hover:text-white/70 font-light leading-relaxed transition-colors duration-500">{it.desc}</p>
              <div className="absolute bottom-0 left-0 right-0 h-[3px] bg-[#F38104] scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left"/>
            </motion.div>
          );})}
        </div>
      </div>
